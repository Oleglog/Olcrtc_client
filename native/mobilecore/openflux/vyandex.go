...
func authorize(docURL string) (*volgaAuth, error) {
	Debugf("[VOLGA] authorize(%s)", docURL)
	...
	for i := 0; i < 10; i++ {
		...
		Debugf("[VOLGA] GET %s -> %d (%d bytes)", currentURL, resp.StatusCode, len(body))
	...
	if len(m) < 2 {
		...
		Debugf("[VOLGA] HTML preview: %s", preview)
	...
	Debugf("[VOLGA] client-config keys: %v", mapKeys(cfg))
	...
	Debugf("[VOLGA] office keys: %v", mapKeys(office))

	actionURL := getStr(office, "action_url")
	accessToken := getStr(office, "access_token")
	ttl := office["access_token_ttl"]

	Debugf("[VOLGA] action_url: %s", actionURL)
	Debugf("[VOLGA] access_token: %d bytes", len(accessToken))
	Debugf("[VOLGA] access_token_ttl: %v (%T)", ttl, ttl)
	...
	Debugf("[VOLGA] ttl formatted: %q", ttlStr)
	...
	Debugf("[VOLGA] POST %s (body %d bytes)", actionURL, len(body))
	...
	Debugf("[VOLGA] auth/initial -> %d", resp2.StatusCode)
	...
	Debugf("[VOLGA] Location: %s", location[:minInt(len(location), 300)])

	if strings.Contains(location, "/document/error/") {
		return nil, fmt.Errorf("auth/initial returned /document/error/ вЂ” check access_token_ttl and Referer")
	}
	...
	Debugf("[VOLGA] auth OK: user=%d(%s) rp=%s sign=%s ts=%s",
...
func (r *relayClient) Start() {
	...
	Debugf("[VOLGA] relay pool started: %d workers, batch=%d timeout=%v",
...
func (r *relayClient) worker(id int) {
	...
	flush := func() {
		...
		if err != nil {
			...
			Debugf("[VOLGA] batch send failed: %v", err)
		} else {
...
func (w *wsListener) run() {
	...
	for {
		...
		Debugf("[VOLGA] WS reconnect in %v", delay)
		select {
...
func (w *wsListener) connect() error {
	...
	Debugf("[VOLGA] WS connected: user=%s", w.auth.UserIDStr)
...
func (t *YandexVolgaTransport) Start() error {
	...
	Debugf("[VOLGA] transport started: user=%d(%s) rp=%s",
...
func (t *YandexVolgaTransport) Stats() TransportStats {
...
func (t *YandexVolgaTransport) statsLoop() {
	...
	for {
		...
		case <-ticker.C:
			...
			Debugf("[VOLGA-STATS] send %d pkt/s (%d KB/s) | http %d req/s fail %d | batch %d (avg %.1f pkt) | recv %d pkt/s (%d KB/s) | busy %d/%d",
